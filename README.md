...
## New: Theme, Organisation Mode & Feature Toggles

The Admin dashboard now has **Theme & Features**. A beginner can change:

- Organisation type (Creative Studio, School, Office, Shop, Restaurant, etc.)
- Main background, cards, text, borders and accent colours
- Separate colours for Hero, About, Projects, Artboard, Contact and Footer sections
- Body font and heading font
- Border/corner radius
- Hero and page labels
- Navigation labels
- About, Projects, Artboard and Contact text
- Footer description and organisation name
- Enable/disable Projects
- Enable/disable Artboard
- Enable/disable Client Portal
- Enable/disable Contact Form
- Enable/disable Invoices
- Enable/disable Client Messaging
- Enable/disable Work Tracking

The settings are stored in `site_settings.value` as JSON. The public pages read these settings when they load.

## Client Profile Picture

Clients can open **Client Portal → My profile** and upload a profile picture. The picture is stored in Supabase Storage under a folder belonging to that authenticated user. The `supabase-client-profile-theme.sql` file adds the required profile fields and Storage policies.

## Additional Supabase setup

After the original schema files have been run, also run:

`supabase-client-profile-theme.sql`

This adds the client avatar fields and secure client-owned avatar upload/update/delete policies.

## Changing the website for another organisation

1. Log into Admin.
2. Open **Theme & Features**.
3. Change **Organisation type**.
4. Change the navigation and page labels.
5. Replace the Hero/About/Projects/Contact wording.
6. Choose the colours for each section.
7. Choose the body and heading fonts.
8. Turn off features that organisation does not need.
9. Save.
10. Refresh the public website.

For a school, for example, you can rename Projects to **Programs**, Artboard to **Gallery**, and Contact to **Admissions**. For a shop, you can rename Projects to **Products** and use the same system as a simple catalogue. The existing database-backed project/gallery system remains available for uploaded content.
