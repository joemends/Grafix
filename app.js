...
async function renderPublicSite() {
  const appearanceSettings = await getSettings();
  if (typeof applySiteAppearance === 'function') applySiteAppearance(appearanceSettings.site || {});
  ...
  const hero=document.querySelector('.hero'); if(hero && merged.hero_image) hero.style.backgroundImage=`linear-gradient(rgba(23,23,23,.45),rgba(23,23,23,.96)),url("${String(merged.hero_image).replace(/"/g,'\\"')}")`;
